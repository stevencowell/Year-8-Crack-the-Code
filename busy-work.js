(() => {
  ...
  function renderChallenge(activity, challenge, challengeIndex) {
    ...
    if (challenge.type === 'table') {
      return `<div class="busy-challenge"><h4>${esc(challenge.title)}</h4><div class="busy-table-wrap"><table class="busy-table"><thead><tr>${challenge.headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${challenge.rows.map((row,i)=>`<tr>${row.slice(0,-1).map(value=>`<td>${esc(value)}</td>`).join('')}<td><input type="text" data-required data-save="${esc(slug(activity,`c${challengeIndex}-${challenge.field}`,i))}"></td></tr>`).join('')}</tbody></table></div></div>`;
    }
    ...
    if (challenge.type === 'matrix') {
      return `<div class="busy-challenge"><h4>${esc(challenge.title)}</h4><div class="busy-table-wrap"><table class="busy-table"><thead><tr><th>Criterion</th><th>Judgement</th><th>Evidence or improvement</th></tr></thead><tbody>${challenge.rows.map((row,i)=>`<tr><td>${esc(row)}</td><td><select data-required data-save="${esc(slug(activity,`c${challengeIndex}-judgement`,i))}">${optionMarkup(['Met','Partly met','Not yet met'])}</select></td><td><textarea data-required data-save="${esc(slug(activity,`c${challengeIndex}-evidence`,i))}"></textarea></td></tr>`).join('')}</tbody></table></div></div>`;
    }
    return '';
  }
  ...
  root.innerHTML = BUSY_WORK_ACTIVITIES.map((activity,index)=>`<details class="busy-activity" id="busy-${String(activity.id).padStart(2,'0')}" data-activity-id="${activity.id}"><summary><span class="busy-number">${String(activity.id).padStart(2,'0')}</span><span><small>Module ${activity.module} · about ${activity.minutes} minutes</small><strong>${esc(activity.title)}</strong><em>${esc(activity.topic)}</em></span><b data-activity-status>Not started</b></summary><div class="busy-activity-body"><p class="lede">${esc(activity.intro)}</p>${activity.challenges.map((challenge,i)=>renderChallenge(activity,challenge,i)).join('')}<div class="busy-reflection"><label for="busy-reflection-${activity.id}">${esc(activity.reflection)}</label><textarea id="busy-reflection-${activity.id}" data-required data-save="${esc(slug(activity,'reflection'))}" placeholder="Write a specific evidence-based response."></textarea></div><div class="busy-actions"><button type="button" data-check-activity>Check and save activity</button><p class="feedback" data-activity-feedback aria-live="polite"></p></div><nav class="busy-pager" aria-label="Busy Work activity navigation">${index?`<a href="#busy-${String(activity.id-1).padStart(2,'0')}">← Previous</a>`:'<span></span>'}<a href="#busy-hub">Back to hub</a>${index<BUSY_WORK_ACTIVITIES.length-1?`<a href="#busy-${String(activity.id+1).padStart(2,'0')}">Next →</a>`:'<span></span>'}</nav></div></details>`).join('');
  ...
  document.querySelectorAll('[data-check-activity]').forEach(button=>button.addEventListener('click',()=>{
    ...
    else if(!result.correct){feedback.className='feedback try';feedback.textContent='Some checked parts need another look. Use the module link, revise the evidence and check again.';}
